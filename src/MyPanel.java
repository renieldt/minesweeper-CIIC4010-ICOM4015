...
public class MyPanel extends JPanel {
	...
	public int minesWanted = 15; //maybe 20 son muchas
	...
	public MyPanel() {   //This is the constructor... this code runs first to initialize
		...
		//Generates mines. If a square or tile is already a mine, generates a mine in a different tile
		for (int i = 0; i < minesWanted; i++){
			for (int j = 0; j < 1; j++){
		...
		for (int x = 0; x < TOTAL_COLUMNS; x++) {
			for (int y = 0; y < TOTAL_ROWS; y++) {
				if ((x == 0) || (y != TOTAL_ROWS)) {
					...
					g.drawString(gridAmount[x][y], x1 + GRID_X + (x * (INNER_CELL_SIZE + 1)) + 10, y1 + GRID_Y + (y * (INNER_CELL_SIZE + 1)) + 20);
	...
	// To determine if a tile contains a mine (Reniel)
